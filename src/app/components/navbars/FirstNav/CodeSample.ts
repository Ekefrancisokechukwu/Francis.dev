...
const Preview = () => {
  ...
  useEffect(() => {
    if (hoveredItem !== null && navRefs.current[hoveredItem]) {
          ...
          {navs.map((nav, i) => (
            <li key={i} className="group">
              <a
                ...
                className="font-medium relative text-slate-800 dark:text-slate-300 w-[8.5rem] px-8 py-2 rounded-3xl flex justify-center"
              >
